...
export async function saveTransactionsFournisseurs(pointId, fournisseursJour, creditsVeille = {}, saisiPar = 'gerant', restaurantId = null) {
  ...
  const { data: fournData } = await supabase
    .from('fournisseurs').select('id, credit_actuel').in('id', fournIds)
  const creditDb = {}
  ;(fournData || []).forEach(f => { creditDb[f.id] = f.credit_actuel || 0 })
  ...
  const lignes = fournIds.map(fournisseurId => {
    ...
    return { point_id: pointId, fournisseur_id: fournisseurId, restaurant_id: restaurantId || null, facture, paye, reste: credit + facture - paye, photo_url: t.photoUri || null, saisi_par: saisiPar, _credit: credit }
  })
...
export async function saveOneTransactionFournisseur(pointId, fournisseurId, transaction, creditVeille = 0, saisiPar = 'gerant', caissierNom = null, restaurantId = null, caissierUserId = null) {
  if (!isOnlineNow()) {
